...
## Made by Mica -- don't contact the original author over this function breaking
## Plays a sound from the beginning, or the given from_position in seconds and using a random pitch.
func play_omni_with_rand_pitch(audio_name: String, from_position: float = 0.0,
								pitch_min: float = 0.75, pitch_max: float = 1.5) -> void:
	var find_audio: AudioStreamPlayer = get_audio_stream_player(audio_name)
	
	if pitch_min <= 0.0:
		pitch_min = 0.01
		push_warning("Pitch minimum should not be equal to or less than 0.")
	##
	
	find_audio.pitch_scale = randf_range(pitch_min, pitch_max)
